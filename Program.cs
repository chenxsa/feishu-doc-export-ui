...
namespace feishu_doc_export
{
    internal class Program
    {
        ...
        public static void RunExportFromGui()
        {
            if (!Directory.Exists(GlobalConfig.ExportPath))
            {
                throw new Exception($"指定的导出目录({GlobalConfig.ExportPath})不存在！！！");
            }
            IOC.Init();
            feiShuApiCaller = IOC.IoContainer.GetService<IFeiShuHttpApiCaller>();
            ExportMainProcess().GetAwaiter().GetResult();
        }

        public static async Task ExportMainProcess()
        {
            // 复用原有主导出流程，命令行和GUI都可调用
            if (!Directory.Exists(GlobalConfig.ExportPath))
            {
